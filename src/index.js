...
var MILLISECONDS_IN_A_SECOND        = PP.define("MILLISECONDS_IN_A_SECOND", 1000),
	ELAPSED_TIME_MAX_DECIMAL_PLACES = PP.define("ELAPSED_TIME_MAX_DECIMAL_PLACES", 3);
...
Animator.prototype = {
	start : function () {
		...
		function event_handler (event) {
			 ...
			 **/
			var elapsed_time = parseFloat(event.elapsedTime.toFixed(ELAPSED_TIME_MAX_DECIMAL_PLACES));
	...
	animator.active = function () {
		...
		self.raf_id = RAF(function () {
			...
			if (self.flags.has_stagger) {
				...
				self.timings.max_delay  = delay * MILLISECONDS_IN_A_SECOND;
			} else {
				self.timings.max_delay  = convert_timing(computed_style.transitionDelay) * MILLISECONDS_IN_A_SECOND;
			}
